...
var datasBloqueadas = await _context.DiaBarbeiros.Where(d => d.BarbeiroId == barbeiroId && !d.Disponivel && d.Data >= DateTime.Today).Select(d => d.Data.ToString("yyyy-MM-dd")).ToListAsync();
...
Console.WriteLine($"BarbeiroNome do agendamento: {model.BarbeiroNome}");
 var bloqueado = await _context.DiaBarbeiros.AnyAsync(d => d.BarbeiroId == emailBarbeiro && d.Data.Date == model.Data.Date && !d.Disponivel);
    ...
    if(bloqueado)
            {
              ModelState.AddModelError("", "O barbeiro está indisponível nesta data.");
    ...
    ViewBag.DatasBloqueadas = datasBloqueadas;
     ...
     return View("Agenda", model);
        ...
        {
            var Barbeiros = await _context.Usuarios.Where(u => u.Perfil == "Barbeiro").Select(u => u.Nome).ToListAsync();
